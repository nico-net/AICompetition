...
if strcmp(insertParameters, "manual")
    ...
    for i = 1:nSignals
        ...
        switch rfSignals(i).SignalType
            case "LE1M"
                rfSignals(i).symRate = 1e6;      % 1 Msym/s :contentReference[oaicite:5]{index=5}
            case "LE2M"
                rfSignals(i).symRate = 2e6;      % 2 Msym/s :contentReference[oaicite:6]{index=6}
            case "BR"
                rfSignals(i).symRate = 1e6;      % 1 Msym/s :contentReference[oaicite:7]{index=7}
            case "802.11ax"
                rfSignals(i).symRate = 20e6;     % 20 MHz BW :contentReference[oaicite:8]{index=8}
            case "802.15.4"
                rfSignals(i).symRate = 4e6;      % 1 Msym/s :contentReference[oaicite:9]{index=9}
            otherwise
        ...
        spsPerSignal = Fs/rfSignals(i).symRate; 
        if (spsPerSignal == floor(spsPerSignal))
            rfSignals(i).SamplesPerSymbol = spsPerSignal;
        else
            rfSignals(i).SamplesPerSymbol = 1;
        end
    end
end
...
%% Generate waveforms
rfConfig = helperRFConfig(RfSignals =rfSignals ,InputSampleRate=sampleRate,...
...
finalWf = createSnapshot(rfConfig);

scope = spectrumAnalyzer(SampleRate=80e6, ViewType="spectrogram");
%scope(finalWf)