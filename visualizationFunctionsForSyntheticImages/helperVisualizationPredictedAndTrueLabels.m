...
%   'ImageSuffix'   - Suffix for image files (default: '_spectrogram.png')
%
% Example:
%   % Basic usage
...
addParameter(p, 'ImageSize', [256, 256], @(x) isnumeric(x) && length(x) == 2);
addParameter(p, 'OldValues', uint8([0, 16, 32, 64, 128]), @isnumeric);
addParameter(p, 'NewValues', [1, 2, 3, 4, 5], @isnumeric);
addParameter(p, 'AutoPause', true, @islogical);
addParameter(p, 'FilePattern', '*_frame.mat', @(x) ischar(x) || isstring(x));
addParameter(p, 'LabelVarName', 'data_final', @(x) ischar(x) || isstring(x));
addParameter(p, 'ImageSuffix', '_spectrogram.png', @(x) ischar(x) || isstring(x));
...
for i = 1:numSamples
    try
        ...
        baseName = regexprep(baseName, '_frame$', '');
        disp(baseName)
