...
for index = 1:length(imageSize)
        ...
        dirName = fullfile('training', folderName);
        if ~exist(dirName, 'dir')
            mkdir(dirName);
        end
end
 
distVec = [1, 2.5, 5, 10, 12, 15, 20];
useGPU = false;
wantPlot = false;
...
for dist = distVec
    for idxFrame = 1:numFrame
        ...
        for i = 1:numel(sigTypes)
            ...
            if i == 1 % we fix the distance of the first smartban signal
                pos = txBatch{index}.findPos(dist);
                txBatch{index}= txBatch{index}.setPos(pos); 
            end
        ...
        overlapLabelledImages(data_tot, idxFrame, dirName, labels, spectrogramTot, signalLabels, dist);    
...
function overlapLabelledImages(data, idxFrame, dir, labels, spectrogram, label_map, dist)
 ...
 % Ensure data is on CPU for processing
    ...
    filename = num2str(idxFrame) + "_" + num2str(dist) + "m_" + ...
                        label_combination;
